...
def distance(v1, v2, d_type='d1'):
  ...
  if d_type == 'd1':# khoảng cách tuyệt đối
    return np.sum(np.absolute(v1 - v2))
  elif d_type == 'd2':# khoảng cách Euclidean
    return np.sum((v1 - v2) ** 2)
  elif d_type == 'd2-norm':# np.dot(v1, v2) = |v1| * |v2| * cos(θ)
    ...
    return 2 - 2 * np.dot(v1, v2)
  elif d_type == 'cosine':# khoảng cách cosine
